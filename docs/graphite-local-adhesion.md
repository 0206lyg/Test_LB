...
`slurry/cases/pure_gr.json`에서 `interaction.local_gap_fraction`만 변경하면
된다. 예를 들어 `1.0`을 `0.8`로 바꾸면 추가 보정 항이 20% 감소한다.
JSON 설정 변경은 다음 새 실행에서 읽으므로 이 값만 바꿀 때는 재빌드가
필요하지 않다. 기존 실행이나 생성된 출력 파일에는 소급 적용되지 않는다.

| alpha | 같은 FF 접촉의 총 순인력 |
| ---: | ---: |
| 1.0 | 936.215 nN |
| 0.8 | 754.586 nN |
| 0.7 | 663.772 nN |
...
같은 간격과 배향에서 추가 보정은 alpha에 선형 비례한다. 기존 RE² 항이
남아 있으므로 전체 순인력은 `28.073 + alpha * (936.215 - 28.073)` nN이다.
전체 suspension 응력은 접촉망과 배향 변화에도 의존하므로 같은 비율로
감소한다고 가정하지 않는다. 이 조절은 D0, sigma, h0 및 마찰계수를 유지한다.

### 강한 부착 접촉망의 수렴 수정

PETSc 선형계 GMRES의 기본 상대 잔차 기준을 0.05에서 0.001로 강화했다.
이는 Newton 방향 계산의 정확도이며, 물리적 힘/토크/간격 허용오차는
바뀌지 않는다. MIT의 108개 입자 실패 상태를 alpha=1 그대로 재현했을 때
기존 설정은 60회 반복 후 정체했고, 수정 설정은 Newton 1회에 통과했다.
재현 입력과 실행법은 `tests/petsc_contact/fixtures/README.md`에 있다.
이 C++ 수정은 아래 빌드 명령으로 재빌드해야 적용된다.
