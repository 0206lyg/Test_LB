...
def read_history(path):
    with path.open(newline="", encoding="utf-8") as source:
        ...
        required = {"step", "time_s", "strain", "eta_wall_Pa_s", "eta_relative", "wall_seconds"}
...
def main():
    ...
    try:
        ...
        summary = {
            ...
            "time_weighted_mean_eta_Pa_s": mean_over_time(selected, "eta_wall_Pa_s"),
            "time_weighted_mean_relative_eta": mean_over_time(selected, "eta_relative"),
            "eta_min_Pa_s": min(row["eta_wall_Pa_s"] for row in selected),
            "eta_max_Pa_s": max(row["eta_wall_Pa_s"] for row in selected),
            ...
            "steady_state_established": False,
        ...
        for name in ("max_mach", "density_drift", "wall_stress_mismatch"):
    ...
    if not args.no_plot:
        try:
            ...
            axes[0].plot([r["time_s"] for r in history], [r["eta_wall_Pa_s"] for r in history], linewidth=1.2)
