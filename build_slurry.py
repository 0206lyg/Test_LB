...
def main():
    ...
    with (build_root / 'build.lock').open('a') as lock:
        ...
        write_json(work / 'build_input_snapshot.json', inputs)
        ...
        if petsc:
            # Make owns the final shell recipe; quote individual tokens there.
            import shlex
            command += ['PETSC_CFLAGS=' + ' '.join(shlex.quote(v) for v in petsc['cflags']),
                        'PETSC_LIBS=' + ' '.join(shlex.quote(v) for v in petsc['libs'])]
        subprocess.run(command, cwd=str(BASE), check=True)
        final_inputs = source_inputs()
        if inputs != final_inputs:
            changed = sorted(k for k in set(inputs) | set(final_inputs) if inputs.get(k) != final_inputs.get(k))
            write_json(work / 'build_changed_inputs.json', {'changed': changed, 'before': inputs, 'after': final_inputs})
            raise RuntimeError('Sources changed while building: ' + ', '.join(changed) +
